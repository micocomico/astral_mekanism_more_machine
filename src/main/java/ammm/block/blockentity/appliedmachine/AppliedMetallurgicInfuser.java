...
public class AppliedMetallurgicInfuser extends BEAppliedEnergizedMachine
    ...
    public AppliedMetallurgicInfuser(IBlockProvider blockProvider, BlockPos pos, BlockState state) {
        super(blockProvider, pos, state, MekanismConfig.usage.metallurgicInfuser.get().multiply(200)
    ...
    private void recalculateRecipeInfo() {
        ...
        if (isA.getItem() instanceof ItemIngredientCardItem cardItemA && isB.getItem() instanceof ChemicalIngredientCardItem cardItemB) {
            ...
            if (keyA != null && keyB != null && hasLevel()) {
                ...
                InfusionStack stackB = AMEKeyUtils.getInfusion(keyB);
